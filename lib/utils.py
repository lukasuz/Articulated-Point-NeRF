...
class InverseProportionalSampler:
  def __init__(self, i_max):
      self.i_max = i_max
      self.counts = np.ones(i_max)
  
  def sample(self, i_min=0, i_max=None):
      i_max = i_max or self.i_max
      range_counts = self.counts[i_min:i_max]
      probabilities = 1 / range_counts
      probabilities = probabilities / np.sum(probabilities)
      index = np.random.choice(np.arange(i_min, i_max), p=probabilities)
      self.counts[index] += 1
      return index
...
def project_point_to_image_plane(points: torch.Tensor, pose: torch.Tensor, intrinsic: torch.Tensor):
    ...
    points = torch.bmm(intrinsic, points.transpose(1, 2)).transpose(1, 2)  # (B, N, 3)
    points = points[:, :, :2] / points[:, :, 2:]  # (B, N, 2)

    return points  # (B, N, 2)

def project_point_to_image_plane2(points: torch.Tensor, pose: torch.Tensor, intrinsic: torch.Tensor):
    """
    Project 3D points to a batch of image planes.
    Args:
        points: (N, 3)
    ...
    points = torch.repeat_interleave(points.unsqueeze(0), len(pose), 0)  # (B, N, 3)
    pose = pose[:,:3,:]
    pose[:,:3,3:] = -torch.bmm(pose[:,:3,:3], pose[:,:3,3:])
    #
    # add homogenous coordinate
    points = torch.cat([points, torch.ones_like(points[:, :, :1])], dim=-1)
    
    points = torch.bmm(pose, points.transpose(1, 2)).transpose(1, 2)
    points = torch.bmm(intrinsic, points.transpose(1, 2)).transpose(1, 2)  # (B, N, 3)
    points = points[:, :, :2] / points[:, :, 2:]  # (B, N, 2)
...
def create_optimizer_or_freeze_model(model, cfg_train, global_step, additional_params=None):
    ...
    for k in cfg_train.keys():
        ...
        if lr > 0:
            ...
            param_group.append({'params': param, 'name': k, 'lr': lr, 'skip_zero_grad': (k in cfg_train.skip_zero_grad_fields)})
        else:
            print(f'create_optimizer_or_freeze_model: param {k} freeze')
            param.requires_grad = False
          
    if additional_params is not None:
      for name, params in additional_params.items():
        lr = getattr(cfg_train, f'lrate_{name}') * decay_factor
        param_group.append({'params': params, 'name': name, 'lr': lr, 'skip_zero_grad': False})
