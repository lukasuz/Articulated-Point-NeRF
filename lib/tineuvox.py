...
class TiNeuVox(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
                 ...
                 feat_only = False, no_view_dir = True,
                 **kwargs):
        ...
        self.register_buffer('xyz_min', torch.tensor(xyz_min).float())
        self.register_buffer('xyz_max', torch.tensor(xyz_max).float())
    ...
    def get_kwargs(self):
        return {
            ...
            'no_view_dir': self.no_view_dir
        }

    @torch.no_grad()
    def get_alpha_mask(self, stepsize, time_sel=torch.tensor([[0.,]]), viewdir=torch.tensor([[0.,0.,0.]]), threshold=None, sampling_freq=1):        
        grid_xyz = torch.stack(torch.meshgrid(
            torch.linspace(self.xyz_min[0], self.xyz_max[0], self.world_size[0] * sampling_freq),
            torch.linspace(self.xyz_min[1], self.xyz_max[1], self.world_size[1] * sampling_freq),
            torch.linspace(self.xyz_min[2], self.xyz_max[2], self.world_size[2] * sampling_freq),
        ), -1).view(-1, 3)

        time_sel = time_sel.to(self.time_poc.device)
        viewdir = viewdir.to(self.view_poc.device)

        times_emb = poc_fre(time_sel, self.time_poc)
        times_feature = self.timenet(times_emb).repeat(grid_xyz.shape[0], 1)

        rays_pts_emb = poc_fre(grid_xyz, self.pos_poc)
        ...
        # alpha = nn.Softplus()(density_result + self.act_shift) # not bounded between 0 and 1?
        # alpha = alpha.squeeze(-1)
        interval = stepsize * self.voxel_size_ratio
        alpha = self.activate_density(density_result, interval)
        if threshold is None and self.fast_color_thres > 0:
            mask = alpha > self.fast_color_thres
        else:
            mask = alpha > threshold

        return grid_xyz.view(*self.world_size, 3), mask.view(*self.world_size)

    def get_grid_xyz(self, sampling_freq, xyz_min=None, xyz_max=None, world_size=None):
        if xyz_min is None:
        ...
        return torch.stack(torch.meshgrid(
            torch.linspace(xyz_min[0], xyz_max[0], int(world_size[0] * sampling_freq)),
            torch.linspace(xyz_min[1], xyz_max[1], int(world_size[1] * sampling_freq)),
            torch.linspace(xyz_min[2], xyz_max[2], int(world_size[2] * sampling_freq)),
        ), -1)

    @torch.no_grad()
    def get_grid_as_point_cloud(self, stepsize, time_sel=torch.tensor([[0.,]]), viewdir=torch.tensor([[0.,0.,0.]]), cam_sel=None, threshold=None, canonical=False, sampling_freq=1, N_batch=2**20, blob_mask=None, alpha_xyz_only=True, grid_xyz=None):        
        if grid_xyz is None:
            grid_xyz = self.get_grid_xyz(sampling_freq)
        og_shape = grid_xyz.shape[:-1]
        grid_xyz = grid_xyz.view(-1, 3)
        if blob_mask is not None:
            blob_mask = blob_mask.view(-1)
    
        time_sel = time_sel.to(self.time_poc.device)
        viewdir = viewdir.to(self.view_poc.device)

        times_emb = poc_fre(time_sel, self.time_poc)
        viewdir_emb = poc_fre(viewdir, self.view_poc)

        batches = list(range(0, len(grid_xyz), N_batch))
        rest = len(grid_xyz) % N_batch
        if rest != 0:
            batches.append(batches[-1] + rest)

        alpha_all = [] 
        rgb_all = []
        h_feature_all = [] 
        vox_feature_flatten_all = []
        grid_xyz_all = []
        binary_volume = []
        alpha_volume = []

        for i in range(len(batches) - 1):
            batch_grid_xyz = grid_xyz[batches[i]:batches[i+1]]
            if blob_mask is not None:
                batch_blob_mask = blob_mask[batches[i]:batches[i+1]]
            viewdir_emb_batch = viewdir_emb

            times_feature = self.timenet(times_emb).repeat(batch_grid_xyz.shape[0], 1)
            if self.add_cam:
                cam_emb = poc_fre(cam_sel, self.time_poc)
                cam_feature = self.camnet(cam_emb)

            rays_pts_emb = poc_fre(batch_grid_xyz, self.pos_poc)
            if canonical:
                ray_pts_shifted = batch_grid_xyz
            else:
                ray_pts_shifted = self.deformation_net(rays_pts_emb, times_feature)
            
            vox_feature_flatten = self.mult_dist_interp(ray_pts_shifted)
            ...
            interval = stepsize * self.voxel_size_ratio
            alpha = self.activate_density(density_result, interval)
            batch_binary_volume = torch.zeros_like(alpha)
            alpha_volume.append(alpha)
            grid_xyz_all.append(batch_grid_xyz)

            # if threshold is None and self.fast_color_thres > 0:
            #     mask = (alpha > self.fast_color_thres)
            #     if blob_mask is not None:
            #         mask = mask & batch_blob_mask
            #     batch_binary_volume[mask] = True
            #     alpha = alpha[mask]
            #     h_feature = h_feature[mask]
            #     batch_grid_xyz = batch_grid_xyz[mask]
            #     vox_feature_flatten_emb = vox_feature_flatten_emb[mask]
            #     vox_feature_flatten = vox_feature_flatten[mask]
            # else:
            #     mask = (alpha > threshold)
            #     if blob_mask is not None:
            #         mask = mask & batch_blob_mask
            ...
            if self.no_view_dir:
                ...
                if self.add_cam:
                    viewdir_emb_batch = torch.cat((viewdir_emb_batch, cam_feature), -1)

                viewdir_emb_batch = viewdir_emb_batch.repeat(h_feature.shape[0], 1)
                rgb_logit = self.rgbnet(h_feature, viewdir_emb_batch)
            rgb = torch.sigmoid(rgb_logit)

            if not alpha_xyz_only:
                alpha_all.append(alpha)
                rgb_all.append(rgb)
                h_feature_all.append(h_feature)
                vox_feature_flatten_all.append(vox_feature_flatten)
                binary_volume.append(batch_binary_volume)

        grid_xyz_all = torch.concat(grid_xyz_all, dim=0)
        alpha_volume = torch.concat(alpha_volume, dim=0).view(*og_shape)

        if not alpha_xyz_only:
            alpha_all = torch.concat(alpha_all, dim=0)
            rgb_all = torch.concat(rgb_all, dim=0)
            h_feature_all = torch.concat(h_feature_all, dim=0)
            vox_feature_flatten_all = torch.concat(vox_feature_flatten_all, dim=0)
            binary_volume = torch.concat(binary_volume, dim=0).view(*og_shape)

            return grid_xyz_all, alpha_all, rgb_all, h_feature_all, vox_feature_flatten_all, binary_volume, grid_xyz.view(*og_shape,3), alpha_volume
        else:
            return  None, None, None, None, None, None, grid_xyz.view(*og_shape, 3), alpha_volume
    ...
    def activate_density(self, density, interval=None, act_shift=None):
        act_shift = act_shift if act_shift is not None else self.act_shift
        interval = interval if interval is not None else self.voxel_size_ratio
        shape = density.shape
        return Raw2Alpha.apply(density.flatten(), act_shift, interval).reshape(shape).squeeze(-1)
    ...
    def forward(self, rays_o, rays_d, viewdirs, times_sel, cam_sel=None, bg_points_sel=None, global_step=None, render_until=None, canonical_t=0, threshold=0.05, **render_kwargs):
        '''Volume rendering
        ...
        # Only warp non canonical points
        # time_mask = (times_sel[ray_id] != canonical_t).squeeze(-1)
        # ray_pts_delta = torch.zeros_like(ray_pts)
        # ray_pts_delta[time_mask] = ray_pts_delta[time_mask] + self.deformation_net(rays_pts_emb[time_mask], times_feature[ray_id][time_mask])
        # inv_time_mask = ~time_mask
        # ray_pts_delta[inv_time_mask] = ray_pts_delta[inv_time_mask] + ray_pts[inv_time_mask]
    ...
    def ray_density(self, rays_o, rays_d, times_sel, cam_sel=None, bg_points_sel=None, global_step=None, threshold=0.1, **render_kwargs):
        '''Volume rendering
        @rays_o:   [N, 3] the starting point of the N shooting rays.
        @rays_d:   [N, 3] the shooting direction of the N rays.
        @viewdirs: [N, 3] viewing direction to compute positional embedding for MLP.
...
def get_rays_of_a_view(H, W, K, c2w, ndc, inverse_y=False, flip_x=False, flip_y=False, mode='center'):
...
@torch.no_grad()
def get_training_rays(rgb_tr, times, train_poses, HW, Ks, ndc, inverse_y=False, flip_x=False, flip_y=False):
...
@torch.no_grad()
def get_training_rays_flatten(rgb_tr_ori, times,train_poses, HW, Ks, ndc, inverse_y=False, flip_x=False, flip_y=False):
...
def get_training_rays_in_maskcache_sampling(rgb_tr_ori, masks_tr_ori, times,train_poses, HW, Ks, ndc, model, render_kwargs, img_to_cam, i_train, inverse_y=False, flip_x=False, flip_y=False, **kwargs):
    ...
    rays_o_tr = torch.zeros(Ks.shape[0] * rgb_tr_ori.shape[1] * rgb_tr_ori.shape[2], 3, device=DEVICE).float()
    rays_d_tr = torch.zeros(Ks.shape[0] * rgb_tr_ori.shape[1] * rgb_tr_ori.shape[2], 3, device=DEVICE).float()
    viewdirs_tr = torch.zeros(Ks.shape[0] * rgb_tr_ori.shape[1] * rgb_tr_ori.shape[2], 3, device=DEVICE).float()
    times_tr = torch.ones([N,1], device=DEVICE)
    times = times.unsqueeze(-1)
    ...
    for c2w, K in zip(train_poses, Ks):
        rays_o, rays_d, viewdirs = get_rays_of_a_view(
    ...
    return rgb_tr, times_tr, rays_o_tr, rays_d_tr, viewdirs_tr, pix_to_ray, masks_tr

def batch_indices_generator(N, BS):
    # More memory friendly
    rng = np.random.default_rng()
    while True:
        yield torch.LongTensor(rng.choice(N, size=BS, replace=False))
